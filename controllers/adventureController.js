...
// @access  Public
export const getTrendingAdventures = async (req, res) => {
    console.log("Fetching trending adventures");
    try {
        const adventures = await AdventurePackage.find({ status: 'Active' })
            .sort({ createdAt: -1 })
            .limit(5);
...
export const getAdventureDetails = async (req, res) => {
    ...
    try {
        const adventure = await AdventurePackage.findOne({ name: req.params.adventure_name });
        if (adventure) {
...
// @access  Public
export const getUpcomingAdventures = async (req, res) => {
    console.log("Fetching upcoming adventures");
    try {
        const today = new Date();
        today.setHours(0, 0, 0, 0); // Set to the beginning of the day

        const adventures = await AdventurePackage.find({
            createdAt: { $gte: today },
            status: 'Active',
        }).sort({ From: 1 });
...
// @access  Public
export const getHomepageUpcomingAdventures = async (req, res) => {
...
export const getAdventuresByCategory = async (req, res) => {
    ...
    try {
        ...
        const adventures = await AdventurePackage.find(query);
...
// @access  Public
export const getAdventureCourses = async (req, res) => {
    console.log("Fetching adventure courses");
    try {
        const courses = await AdventurePackage.find({
...
// @access  Public
export const viewAdventurePackages = async (req, res) => {
    console.log("Fetching all adventure packages");
    try {
        const packages = await AdventurePackage.find({}).sort({ createdAt: -1 });
...
// @access  Public
export const fetchHomepageVendors = async (req, res) => {
...
// @access  Public
export const fetchVendorPackages = async (req, res) => {
    ...
    try {
        ...
        const packages = await AdventurePackage.find({ serviceProvider: serviceProviderId });
